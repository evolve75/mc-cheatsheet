...
\fancyhead[L]{\textsf{Midnight Commander (mc) Cheatsheet}}
\fancyhead[R]{\textsf{Default keys}}
...
% Wrap-friendly code for long protocol-ish strings in tables
% \path provides good breakpoints at / : . _ - etc.
\newcommand{\codewrap}[1]{{\ttfamily\small\path{#1}}}

% ---------- Key table environment (NO tabularx) ----------
% Slightly narrower key column prevents visual overflow in long-action rows.
...
System (VFS) that lets you browse archives and remote hosts (e.g., \key{sftp://}, \key{sh://})
...
\begin{itemize}
  ...
  \item Many terminals map \ks{Esc}+\key{1..9,0} $begin:math:text$\\rightarrow$end:math:text$ \ks{F1..F10}.
  \item \ks{Esc Esc} quickly dismisses menus/popups (gist).
\end{itemize}
...
\sect{Launch \& CLI options (mc(1))}\par
\begin{itemize}
...
\ks{F2} & User menu (custom actions/macros).\\
\ks{F3} & View file (\ks{Shift-F3} raw view; disregard extension).\\
\ks{F4} & Edit file (\ks{Shift-F4} create new file).\\
\ks{F5} & Copy selected/tagged to other panel dir.\\
\ks{F6} & Move selected/tagged (\ks{Shift-F6} rename under cursor).\\
\ks{F7} & Mkdir.\\
\ks{F8} & Delete.\\
...
\begin{keytable}
\ks{Insert}/\ks{C-t} & Select/deselect file.\\
\ks{*} & Invert selection on files.\\
\ks{+} & Selection options (incl.\ custom pattern).\\
...
\ks{A-o} & Show highlighted dir in the other panel.\\
\ks{A-t} & Change panel view (full/brief/long).\\
\ks{A-.} & Toggle “Show hidden files”.\\
\end{keytable}

\sect{Panels menu (F9 $begin:math:text$\\rightarrow$end:math:text$ Left/Right)}\par
\begin{keytable}
...
\ks{Command} & Find file, quick cd, hotlist, compare dirs, external panelize, jobs.\\
\ks{Options} & Configuration, appearance/layout, key learning, confirmations (build dependent).\\
\end{keytable}

\sect{File/directory operations (gist)}\par
\begin{keytable}
...
\ks{C-x v} & Relative symlink tool on selected/tagged.\\
...
\ks{C-x t} & Put selected/tagged item names on command line.\\
...
\ks{A-n} / \ks{A-p} & Move up/down through command history.\\
...
\ks{Example} & \key{ls -1 | ...} (pipelines that produce 1 file per line).\\
\end{keytable}

\sect{Background jobs (gist)}\par
\begin{keytable}
\ks{C-x j} & Jobs list; inspect/cancel background operations.\\
\end{keytable}
...
\sect{User menu macros (mc(1), selected)}\par
\begin{keytable}
\ks{\%f}, \ks{\%p} & Current file (name/path).\\
\ks{\%d}, \ks{\%D} & Current dir / other panel dir.\\
...
\ks{\%view[args]} & Internal viewer (ascii/hex/nroff/...).\\
...
\sect{Keymaps / learn keys}\par
\begin{keytable}
\ks{Learn keys} & Writes terminal sequences into \pathtt{~/.config/mc/ini} under \key{[terminal:TERM]}.\\
\ks{-K}/\ks{--keymap} & Load a specific keymap file.\\
\ks{MC\_KEYMAP} & Env var that selects a keymap.\\
\ks{~/.config/mc/mc.keymap} & User keymap file location (common).\\
\end{keytable}
...
\sect{VFS: browse “special paths” (mc(1))}\par
\begin{keytable}
\ks{Concept} & VFS lets you \emph{cd into} archives/remotes like directories.\\
\ks{C-x a} & List active VFS directories (gist).\\
\ks{Tip} & VFS can stack (e.g., archive stored on an \key{sftp://} path).\\
\end{keytable}

\sect{Remote VFS (mc(1))}\par
\begin{keytable}
\ks{sh://}   & \codewrap{sh://[user@]machine[:port]/[remote-dir]}\\
\ks{sftp://} & \codewrap{sftp://[user@]machine[:port]/[remote-dir]}\\
\ks{Example} & \codewrap{sh://joe@somehost:2222/private}\\
\ks{Example} & \codewrap{sftp://joe@somehost:2222/private}\\
\end{keytable}

\sect{Archives / EXTfs (mc(1))}\par
\begin{keytable}
\ks{Example} & \codewrap{cd documents.zip/uzip://} (mc(1) example).\\
\ks{Pattern} & \codewrap{cd file.tar.gz/utar://} (extfs name varies).\\
\ks{Standalone} & \codewrap{cd fsname://} (enter an extfs “filesystem”).\\
\end{keytable}

\sect{Undelete (optional Linux ext2)}\par
\begin{keytable}
\ks{undel://} & \codewrap{undel://DEVICEPART} (e.g., \codewrap{undel://sda2}) if built with support.\\
\end{keytable}

\columnbreak

\sect{Configuration “what lives where” (mc(1))}\par
\begin{keytable}
...
\sect{Subshell notes (mc(1))}\par
\begin{keytable}
\ks{-u / -U} & Disable / force subshell.\\
\ks{Concept} & Subshell shares your shell env/aliases while mc runs.\\
\ks{bashrc} & Commonly \pathtt{~/.local/share/mc/bashrc} (varies by install).\\
\end{keytable}

\sect{Troubleshooting quick hits}\par
\begin{keytable}
\ks{F10 stolen} & Quit via menu (\ks{F9} $begin:math:text$\\rightarrow$end:math:text$ Quit) or remap terminal key.\\
\ks{Alt combos} & Use \ks{Esc} then key (Meta emulation).\\
\ks{tmux/screen} & Try \ks{mc -g}; re-run “Learn keys”.\\
\ks{Display odd} & Try \ks{mc -a} (ASCII) or adjust terminal settings.\\
\end{keytable}

\sect{References (URLs)}\par
\begin{keytable}
\ks{mc(1)} & \tinyurl{https://source.midnight-commander.org/man/mc.html}\\
