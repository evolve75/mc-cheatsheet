...
%
% ----------------------------------------------------------------------
% PRINT CHECKLIST (TL;DR)
%
% 1. Build:
%      latexmk -pdf -xelatex mc-cheatsheet.tex
%
% 2. Impose booklet (DO NOT skip this step):
%      pdfbook2 --paper=letterpaper --short-edge mc-cheatsheet.pdf
%      mv mc-cheatsheet-book.pdf mc-cheatsheet-booklet.pdf
%
% 3. Print:
%    - File:        mc-cheatsheet-booklet.pdf
%    - Paper:       US Letter
%    - Duplex:      ON
%    - Binding:     Short-edge
%    - Scaling:     100% (no scaling)
%    - Orientation: Auto
%    - Printer-side booklet mode: OFF
%
% 4. After printing:
%    - Fold at center
...
% ---------- Page geometry (lamination-safe, booklet gutter-aware) ----------
% For a folded duplex booklet (imposed externally via pdfbook2), reserve a bit
% more whitespace on the *inner* edge of each logical page for the fold/gutter.
\usepackage[
  inner=0.65in,
  outer=0.45in,
  bindingoffset=0.10in,
  top=0.75in,
  bottom=0.75in
]{geometry}
...
% Slightly more breathing room between the two columns.
\raggedcolumns{}
...
% ---------- Color palette (enhanced contrast, still print-safe) ----------
\definecolor{MCBlue}{HTML}{0B3C5D}   % deeper navy-blue (headings, rules)
\definecolor{MCGray}{HTML}{555555}   % darker gray (headers/footers)
\definecolor{MCKey}{HTML}{002A4E}    % strong blue for keybindings
\definecolor{MCCode}{HTML}{1A1A1A}   % near-black for code
...
% Two-sided: keep the title on the outer edge, label on the inner edge.
\fancyhead[LE,RO]{\textsf{\color{MCGray}Midnight Commander (mc)}}
\fancyhead[LO,RE]{\textsf{\color{MCGray}Cheatsheet}}
% Footer: apply the same two-sided logic (page number on the outer edge).
\fancyfoot[LE,RO]{\textsf{\color{MCGray}Page \thepage\ of 4}}
...
\newcommand{\key}[1]{\texttt{\color{MCCode}#1}}
\newcommand{\ks}[1]{\textbf{\texttt{\color{MCKey}#1}}}
\newcommand{\sect}[1]{\textsf{\color{MCBlue}\textbf{#1}}}
...
\newcommand{\codewrap}[1]{{\ttfamily\small\color{MCCode}#1}\space}
% Path/protocol strings: allow breaks at / : . _ - etc., and preserve trailing space
\newcommand{\codepath}[1]{{\ttfamily\small\color{MCCode}\path{#1}}\space}
...
{%
  ...
  \toprule
  \textsf{\color{MCGray}Key} & \textsf{\color{MCGray}Action}\\
  \midrule
}
{%
  \bottomrule
...
\begin{itemize}
  ...
  \item Many terminals map \ks{Esc}+\key{1\ldots{}9,0} $begin:math:text$\\rightarrow\{\}$end:math:text$ \ks{F1\ldots{}F10}.
...
% (balanced automatically by multicols)
